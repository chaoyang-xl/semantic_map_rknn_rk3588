...
为了验证“最终只需要 2D 地图，是否可以从投影阶段就压平”的方案，曾临时实现
二维跟踪实验。二维模式仍先完整应用
`camera -> map` 位姿，然后将 map 坐标系中的 Z 设为 0；因此 XY 坐标没有绕过
SLAM 位姿变换，但后续体素、聚类、关联和融合都只保留平面几何。

两组实验在同一块 OrangePi、同一时段按顺序运行，均使用本报告第 7 节参数，且
输入检测和成功投影均为 6159：

| 跟踪空间 | 总耗时/s | Fusion/s | 最终点数 | 确认对象 |
| --- | ---: | ---: | ---: | ---: |
| 3D（默认） | 669.125 | 344.660 | 85610 | 19 |
| 2D（实验） | 675.624 | 223.698 | 33267 | 21 |

结果说明：

- 2D 将 Fusion 累计工作量降低 35.09%，最终点数降低 61.14%。
- 总墙钟时间反而增加 0.97%。流水线中的 YOLO、SAM 和 IO 主路径没有因 Z 被
  压平而减少，节省的 CPU 工作被 NPU 阶段重叠，并没有转化为端到端吞吐收益。
- 2D 产生 5 个 chair 和 5 个 vase，3D 分别为 4 个和 4 个。两边检测次数完全
  相同，额外对象来自几何关联拆分，不是模型新增检测。
- 压平会丢失高度层次，使不同表面在 XY 上重合，同时改变空间聚类、体素去重和
  几何重叠率。它适合性能剖析或仅需粗略占地区域的应用，不适合作为当前默认追踪
  表示。
...
RGB-D + map pose
  ...
  -> 3D 几何关联与融合
  -> confirmed 对象点云/JSON
  -> 最终阶段投影到 XY 生成 2D 语义地图
```

这样保留 3D 对象身份和几何约束，而最终导航接口仍使用轻量的 2D 表达。由于
实验没有带来端到端性能收益且降低了关联精度，二维跟踪实现和对应命令参数已经
从当前代码中回退；本节仅保留测试方法、数据和结论。
